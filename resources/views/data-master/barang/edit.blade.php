<x-layouts.dashboard :title="$title" :breadcrumbs="$breadcrumbs">
    ...
    <div class="card shadow mb-4" id="card-form-{{ $title }}">
        ...
        <div class="card-body">
            <form enctype="multipart/form-data" method="POST" action="{{ route('barang.update', $barang->id) }}"
                ...
                <div class="row">
                    ...
                    <div class="col-6 mb-3">
                        <label for="fotoBarang" class="form-label font-weight-bold">Foto Barang
                            (Opsional)</label>
                        ...
                        @enderror
                        <img id="preview"
                            src="{{ $barang->foto_barang ? asset('storage/' . $barang->foto_barang) : '#' }}"
                            alt="Preview Foto Barang" class="img-thumbnail mt-2"
                            style="{{ $barang->foto_barang ? '' : 'display: none;' }} max-height: 150px;">
                    </div>
                    ...
                    <div class="col-6 mb-3">
                        <label for="stokFinal" class="form-label font-weight-bold">Stok Barang Awal<span
                                class="text-danger">*</span></label>
                        <input type="number" name="stok_final" value="{{ old('stok_final', $barang->stok_final) }}"
                    ...
                    <div class="col-12 mb-2">
                        <label for="deskripsi" class="form-label font-weight-bold">Deskripsi
                            (Opsional)</label>
                        <textarea name="deskripsi" class="form-control @error('deskripsi') is-invalid @enderror" id="deskripsi"
                            rows="3" placeholder="Masukkan Deskripsi Barang">{{ old('deskripsi', $barang->deskripsi) }}</textarea>
                        @error('deskripsi')
                            <div class="valid-feedback">
                                {{ $message }}
                            </div>
